...
missing_mechanisms_env <- Sys.getenv("MISSING_MECHANISMS", unset = "mar,time_mar")
...
allowed_missing_mechanisms <- c("mar", "time_mar")
if (length(missing_mechanisms) == 0L || any(!missing_mechanisms %in% allowed_missing_mechanisms)) {
  stop(
    "MISSING_MECHANISMS must contain values from: ",
    paste(allowed_missing_mechanisms, collapse = ", "),
    call. = FALSE
  )
}
...
task_result_path <- function(task) {
  file.path(
    task_result_dir,
    sprintf(
      "scenario%d_%s_miss%02d_n%d_d%d_rep%d.rds",
      task$scenario_id,
...
apply_missingness <- function(dat, rate, mechanism, seed) {
  ...
  if (rate <= 0) {
    ...
    attr(dat, "missingness_summary") <- missingness_summary(dat, mechanism, rate)
    return(dat)
  }
  ...
  linear_part <- switch(
    mechanism,
    mar = 0.70 * scale(dat$x1)[, 1] + 0.50 * dat$x2 - 0.40 * scale(dat$s1)[, 1],
    time_mar = 0.70 * scale(dat$x1)[, 1] + 0.50 * dat$x2 - 0.40 * scale(dat$s1)[, 1] +
      1.35 * scale(dat$t)[, 1],
    stop("Unknown missingness mechanism: ", mechanism, call. = FALSE)
  )
  linear_part[!is.finite(linear_part)] <- 0
  intercept <- calibrate_missing_intercept(linear_part, rate)
  miss_prob <- stats::plogis(intercept + linear_part)
  is_missing <- stats::runif(nrow(dat)) < miss_prob
  dat$missing_probability <- miss_prob
  dat$response_observed <- !is_missing
  dat$response[is_missing] <- NA_real_
  attr(dat, "missingness_summary") <- missingness_summary(dat, mechanism, rate)
  dat
}

missingness_summary <- function(dat, mechanism, target_rate) {
  dat_ordered <- dat[order(dat$id, dat$time), , drop = FALSE]
  pairs <- adjacent_pair_rows(dat_ordered)
  complete_pairs <- if (nrow(pairs) == 0L) {
    0L
  } else {
    sum(is.finite(dat_ordered$response[pairs$row1]) & is.finite(dat_ordered$response[pairs$row2]))
  }
  ...
  data.frame(
    ...
    complete_adjacent_pair_rate = if (total_pairs > 0L) complete_pairs / total_pairs else NA_real_,
    stringsAsFactors = FALSE
  )
}
...
run_one_rep <- function(task) {
  ...
  dat <- apply_missingness(
    ...
    seed = seed +
      (if (identical(missing_mechanism, "mar")) 250000 else 350000) +
...
if (!is.null(smooth_all) && nrow(smooth_all) > 0) {
  ...
  smooth_irmse_summary$target_missing_pct <- 100 * smooth_irmse_summary$target_missing_rate
  smooth_irmse_summary$model <- factor(
    smooth_irmse_summary$model,
    levels = c("gamlss2", "gamlss.longitudinal")
  )
  plot_smooth_irmse <- ggplot(
    smooth_irmse_summary,
