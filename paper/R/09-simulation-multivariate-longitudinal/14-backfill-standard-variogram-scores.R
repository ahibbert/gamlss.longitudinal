...
methods <- mvt_env_vector(
  "GAMLSS_LONGITUDINAL_MVT_BACKFILL_METHODS",
  c("glm", "glmm", "gee_independence", "gee_exchangeable", "gee_ar1", "gee_unstructured")
)
methods <- intersect(methods, mvt_allowed_comparators())
methods[methods == "gamCopula"] <- "gamCopula_markov"
methods <- unique(methods)
if (length(methods) == 0L) {
  stop("No valid methods requested for variogram backfill.", call. = FALSE)
}

case_ids <- mvt_env_vector("GAMLSS_LONGITUDINAL_MVT_BACKFILL_CASE_IDS", character())
...
max_cases <- mvt_env_int("GAMLSS_LONGITUDINAL_MVT_BACKFILL_MAX_CASES", 0L)
...
message("Backfilling variogram scores in: ", run_dir)
...
current <- mvt_read_optional_csv(path)
checkpoint_every <- mvt_env_int("GAMLSS_LONGITUDINAL_MVT_BACKFILL_CHECKPOINT_EVERY", 5L)
...
for (i in seq_len(nrow(grid))) {
  ...
  out <- mvt_run_case(grid[i, , drop = FALSE], seed_base = seed_base, require_gamcopula = FALSE)
  new_rows <- out$variogram_scores[out$variogram_scores$method %in% methods, , drop = FALSE]
  if (nrow(new_rows) > 0L) {
    if (nrow(current) > 0L && all(c("case_id", "method") %in% names(current))) {
      keep <- !(current$method %in% methods & current$case_id == grid$case_id[[i]])
      current <- mvt_bind_rows_fill(current[keep, , drop = FALSE], new_rows)
    } else {
      current <- mvt_bind_rows_fill(current, new_rows)
    }
  }
  checkpoint_case_ids <- c(checkpoint_case_ids, grid$case_id[[i]])
  if (checkpoint_every > 0L && i %% checkpoint_every == 0L) {
    mvt_write_csv(current, path)
...
append_progress(checkpoint_case_ids)
mvt_summarise_results(run_dir)
message("Backfilled variogram scores written to: ", path)
