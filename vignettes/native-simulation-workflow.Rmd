...
```{r marginal-fit-overlay, fig.alt = "Histograms of observed responses comparing generalized gamma and gamma marginal fits with time-intercept overlays."}
...
Linear time for `sigma` removes three degrees of freedom and provides the same AIC and better BIC so we select it as the improved model. The covariate is highly significant.
...
We provide a brief automated model assessment using `check_model()` and return either a concern or a clear check. Check model is readonably minimal and should be used alongside other diagnostics, the checks that flag are:

 | Area | Quantity checked | Threshold / condition | Default | Severity | Recommendation |
|---|---:|---:|---:|---|---|
| Convergence | `object$convergence$converged` | Not `TRUE` | n/a | `concern` | `revise_before_primary` |
| Marginal calibration | PIT Kolmogorov-Smirnov p-value vs Uniform(0, 1) | `ks_p_value < 0.05` | `0.05` | `concern` | `revise_before_primary` |
| Tail calibration | Maximum of lower/upper PIT tail ratios across thresholds `0.05` and `0.10` | `max(lower_ratio, upper_ratio) > 2` | `2` | `review` | `primary_candidate_with_caveats` unless a `concern` also exists |
| Residual dependence | Absolute lag-1 Rosenblatt normal-score residual correlation after fitted copula | `abs(lag1_cor) > dependence_cor_cutoff` | `0.25` | `concern` | `revise_before_primary` |
| Inference | Variance-covariance method from summary | `vcov_method == "numderiv"` | n/a | `note` | `primary_candidate_with_caveats` unless a `concern` also exists |
| Overall | No automatic checks triggered | No `concern`, `review`, or `note` rows | n/a | `ok` | `primary_candidate` |
...
```{r fit-diagnostics-margin, fig.width = 6, fig.height = 9, out.width = "95%"}
plot(fit)
```

Marginal diagnostics for PIT and rootogram look reasonable. Work plot and QQ plot indicate some deviation from the fitted distribution in the lower tail that could be investigated further.

```{r fit-diagnostics-dependence}
plot_copula_diagnostics(fit, fig.width = 9, fig.height = 9, out.width = "95%")
```

All trends for fitted versus observed dependence appear close. There is s ome small deviation between the fit and the data in the lower tail that can be seen in the Rosenblatt normal QQ plot and the tail co-occurrence / exceedence plots for the lower plot where we are slightly underfitting it appears.

We can also plot the final fitted distributions against the original data using `plot_margin_fit(fit, by_time=TRUE)` and `plot_copula_fit(fit, by_time=TRUE)` if desired.
