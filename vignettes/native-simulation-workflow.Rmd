...
if (!requireNamespace("gamlss.longitudinal", quietly = TRUE) &&
...
plot_copula_screen_overlay <- function(pair_data, selected_fit, grid_n = 80, bins = 28) {
  ...
  density_grid$density <- gamlss.longitudinal:::.copula_pdf(
    density_grid$u1,
    density_grid$u2,
    family = selected_fit$family[1],
    par = selected_fit$par[1],
    par2 = selected_fit$par2[1]
  ) * stats::dnorm(density_grid$z1) * stats::dnorm(density_grid$z2)
  ...
  ggplot(pair_data, aes(x = z1, y = z2)) +
    ...
    labs(
      title = paste("Selected copula:", selected_fit$family[1]),
