...
We provide a brief descriptive diagnostic summary using `check_model()`. It should be used alongside graphical diagnostics and subject-matter review; calibration statistics are not package pass/fail verdicts.
...
| Marginal fit | PIT KS summary | Descriptive; review marginal plots | `descriptive` |
| Tail fit | PIT tail ratios | Descriptive; interpret in context | `descriptive` |
| Copula fit | Rosenblatt residual correlation by lag | Descriptive unless the user supplies a threshold | `descriptive` or user-defined flag |
| Variance calculation | Covariance-method provenance | Report the inference contract | provenance/review status |

For discrete margins the PIT is randomized reproducibly using the recorded `pit_seed`, without changing the caller's random-number state. Requested higher lags that cannot be assessed remain in the result with an explicit unavailable scope status.
...
- PIT appears reasonably close to uniform, with small deviation in the lower tail; the PIT KS value is treated as a descriptive summary
...
The package can generate conditional Wald outputs for fixed coefficients when the fit-level inference checks succeed. These results condition on the fitted smooth structure and omit fixed-smooth covariance and smoothing-parameter uncertainty; they are not a blanket validation of inference for every parameter or model configuration.
...
Mean-only GEEs and GLMMs target different estimands and can be useful comparators when the primary question concerns the mean and their distributional and correlation assumptions are suitable. The example below compares selected predictive and distributional metrics for this one design; it does not establish general confidence-interval validity or superiority for other families, dependence structures, or sample sizes.
...
- In this illustrative design, `gamlss.longitudinal` has the strongest values for several reported distributional metrics, including interval coverage, tail error, and log score
- `glmer` and `gamm` provide the best fits for MAE and RMSE against the observed response
- Coefficients are broadly in the same range in this example, but standard errors and 95% intervals differ materially because the models target different likelihoods and parameterizations. These differences should be interpreted as design-specific evidence; broader conditional-coverage work is required before making general inferential claims.
