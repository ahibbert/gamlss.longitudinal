...
- use `predict(..., se.fit = TRUE, interval = "confidence")` for
  response-mean intervals, and `predict()` more broadly for any row-level outputs like prediction intervals
...
Since we are using the inverted Hessian directly, these intervals are large-sample approximations. A comparison of the intervals to the bootstrap intervals should give a good test for the sensitivity of this assumption.
...
Note that by default it is not possible for the prediction, if it's on new datasets, to preserve the structure of the correlation. Essentially `predict()` on a new dataset is providing a marginal fit and should not be relied on for correlation analysis of the predicted object, though the marginal confidence interval predictions will be taking into account the impact of the fitted correlation structure on the confidence intervals.
