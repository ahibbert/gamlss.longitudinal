...
test_that("benchmark_standard_models can score the supplied primary fit", {
  ...
  expect_s3_class(bench$model_status, "data.frame")
  expect_true(all(c("method", "estimator", "success") %in% names(bench$model_status)))
  ...
  expect_false(any(grepl("Model Status", printed, fixed = TRUE)))
  expect_true(any(grepl("Methods included:", printed, fixed = TRUE)))
  expect_true(any(grepl("Benchmark Details", printed, fixed = TRUE)))
  expect_true(any(grepl("Best model", printed, fixed = TRUE)))
  expect_true(any(grepl("Mu Coefficients", printed, fixed = TRUE)))
  expect_true(any(grepl("Coefficient Estimates", printed, fixed = TRUE)))
  expect_true(any(grepl("Standard Errors", printed, fixed = TRUE)))
  expect_true(any(grepl("95% Lower Estimates", printed, fixed = TRUE)))
  expect_true(any(grepl("95% Upper Estimates", printed, fixed = TRUE)))
