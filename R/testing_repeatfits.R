...
library("gamlss"); library("VineCopula");library("moments")
#library(gamlss.longitudinal)
options(scipen=999)
set.seed(1000)

#Libraries just for the outer loop to use multiple cores
...
out=foreach(run_counter=1:sims,.packages = c("VineCopula","gamlss","moments","gee"),.errorhandling = 'remove') %dopar% {
  ...
  no_dl=fit_jointreg(dataset, margin_dist,copula_dist,
                     mu.formula = mu_formula, sigma.formula = ("~ 1"), nu.formula = ("~ 1"), tau.formula = ("~ 1"),
  ...
  if(fit_gee==TRUE) {
    ...
    if(margin_dist$family[1]=="GA") {
      gee_fit=gee(formula=mu_formula,data=dataset[order(dataset$subject,dataset$time),],family=Gamma(link="log"),id=subject,corstr="exchangeable")
    } else {
      gee_fit=gee(formula=mu_formula,data=dataset[order(dataset$subject,dataset$time),],family=gaussian,id=subject,corstr="exchangeable")
    }
  }
