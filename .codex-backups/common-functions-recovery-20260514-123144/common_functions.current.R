...
#' `compute_vcov = TRUE`. Retained for backwards compatibility.
                      ...
                      )
{
  ...
  if(!is.finite(inner_stop_crit)) inner_stop_crit <- 0.1
  if(!is.finite(outer_stop_crit)) outer_stop_crit <- if(method == "CG") 0.01 else 0.1
  if(!is.finite(lambda_start)) lambda_start <- 5
  ...
  if(!is.finite(cg_max_delta) || cg_max_delta <= 0) cg_max_delta <- 1
  if(!is.finite(cg_max_delta_upper) || cg_max_delta_upper < cg_max_delta) cg_max_delta_upper <- cg_max_delta
  cg_grad_tol_eff <- if(!is.finite(cg_grad_tol)) max(1e-3, 10 * outer_stop_crit) else cg_grad_tol
  cg_step_tol_eff <- if(!is.finite(cg_step_tol)) max(1e-5, 0.1 * outer_stop_crit) else cg_step_tol
        ...
        if(!par_name %in% c("mu","sigma","nu","tau")) {
          if(par_name == "theta") {
            ...
            if(length(row_id1)>0) {
              ...
              if(any(valid_idx)) {
                d1_full[,1] <- rowsum(dldth[valid_idx], par_idx[valid_idx], reorder = FALSE)[as.character(seq_len(n_par)), 1]
                d1_full[!is.finite(d1_full)] <- 0
              }
            }
            ...
            if(length(row_id1)>0) {
              ...
              valid_idx <- is.finite(par_idx) & par_idx >= 1 & par_idx <= nrow(d1_full)
              if(any(valid_idx)) {
                d1_full[,1] <- rowsum(dldz[valid_idx], par_idx[valid_idx], reorder = FALSE)[as.character(seq_len(n_par)), 1]
                d1_full[!is.finite(d1_full)] <- 0
              }
            }
            d1=as.matrix(d1_full)
...
logLik.gamlss.longitudinal=function(object, ...) {
  ...
  if(!is.null(object$df_s)) {
    df_s_vals <- suppressWarnings(as.numeric(unlist(object$df_s, use.names = FALSE)))
    df_s_vals <- df_s_vals[is.finite(df_s_vals)]
    attr(ll, "df") <- length(object$par) + sum(df_s_vals)
  }
  return(ll)
}
...
get_starting_values = function(copula_dist,margin_dist,dataset,eta_transform=FALSE) {
  ...
  cop_param_names <- get_copula_dist(copula_dist)$parameters
  if(length(cop_par) < length(cop_param_names)) {
    default_extra <- rep(0, length(cop_param_names) - length(cop_par))
    names(default_extra) <- cop_param_names[(length(cop_par) + 1):length(cop_param_names)]
    if("zeta" %in% names(default_extra)) {
      default_extra["zeta"] <- 10
    }
    cop_par <- c(cop_par, default_extra)
  }
  cop_par <- cop_par[seq_along(cop_param_names)]
  names(cop_par)=cop_param_names
